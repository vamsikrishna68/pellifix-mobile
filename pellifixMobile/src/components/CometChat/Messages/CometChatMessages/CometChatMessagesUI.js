...
class CometChatMessagesUI extends React.PureComponent {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const prevParams = prevProps.location;
  ...
  actionHandler = (action, messages, key, group, options) => {
    ...
    switch (action) {
      ...
      case actions.GO_BACK:
        if (params && params.routeInfo && params.routeInfo.title == 'Chat') {
          this.props.navigate('/auth/chat',{
            state:{
              ...params
            }
          });
        } else {
          this.props.navigate('/auth/home');
        }
        break;
...
export default CometChatMessagesUI;
